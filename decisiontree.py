...
"""
classification = clf.predict(unlabeled)



print(clf.tree_.threshold) #clf.tree_.value, clf.tree_.children_left, clf.tree_.children_right)

for index, sample in enumerate(unlabeled):
	print("%s --- %s" % (classification[index], treeClassify(clf, sample)))
